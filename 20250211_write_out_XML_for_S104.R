## Latest attempt to write out a Catalogue XML file for S-104
## 2025-01-30

Sys.setenv(TZ='UTC')
library(XML)
source("Z:/individual/thocro/github/hdf5/s100_hdf5_testing/rscripts/s100_testing_utility_functions.R")
...
setwd("Z:/individual/thocro/scratch_not_github/104GB00_20250210T001500Z_UKNWS_dcf2 (1)/")
list_of_h5_files <- list.files(pattern = ".h5")

## choose a name for the S100XC:identifier section near the top of the file
exchange_set_identifier <- "S-104_ROUTE_FEB25"

## downloaded and unzipped from the UKHO file share service
hd_file <- suppressWarnings(hdf5_limits_and_check(s104_or_s111 = "s104",list_of_h5_files[1]))
                            
## set which version of S-104, this could be read from the file dynamically but 
## for now i prefer to hardcode it here
s104_v1 <- "1.1.0"
s104_v1_1 <- as.numeric(substr(s104_v1,1,1))
s104_v1_2 <- as.numeric(substr(s104_v1,3,3))
s104_v1_3 <- as.numeric(substr(s104_v1,5,5))
...
newXMLNode("gco:CharacterString", "s104v1", parent = comment) # hardcoded change if neccessary
...
for (i in seq(1,length(list_of_h5_files))) {
  ...
  # Read in File
  hd_file <- suppressWarnings(hdf5_limits_and_check(s104_or_s111 = "s104",file_name))
  ...
  dataset_id <- sprintf("urn:mrn:iho:s104:%01d:%01d:%01d:GB:%s:type2:%s%02d%02dT%02d%02d%02d+0000",1,1,0,fname,y1,m1,d1,h2,mn1,0)
  
  metadata_item <- newXMLNode("S100XC:S100_DatasetDiscoveryMetadata", parent = metadata)
  newXMLNode("S100XC:fileName", sprintf("file:/S-104/DATASET_FILES/%s",file_name), parent = metadata_item)
  ...
  ## Ignoring Digital Signature Bit for Now
  
  # newXMLNode("S100XC:digitalSignatureReference", "ECDSA-384-SHA2", parent = metadata_item)
  # signature <- newXMLNode("S100XC:digitalSignatureValue", parent = metadata_item)
  # newXMLNode("S100SE:S100_SE_DigitalSignature", 
  #            "MGUCMDTwr53voYs18+b6pw8175V3i4XW+rHQRSe0fpwQAPbJ2YpOqKTINtfGWSvm8vWiogIxAL/TJLzuY6rt69k7W/oT9tAfqqHFKLyA6xmIGqz4PDkR3Bwz5jtfpzZAGuy4+QyoXg==", 
  #            attrs = c(id = "SIG1042C00TDS024MV01_20231231T141000Z", 
  #                      certificateRef = "urn:mrn:iho:GB00:540"), 
  #            parent = signature)
  ...
  newXMLNode("S100XC:name", "Water Level Information for Surface Navigation", parent = productSpec)
  newXMLNode("S100XC:version", sprintf("%01d.%01d.%01d",s104_v1_1,s104_v1_2,s104_v1_3), parent = productSpec)
  newXMLNode("S100XC:productIdentifier", "S-104", parent = productSpec)
